 ...
 * @param checkFunc function to be called when an item from Tasks list is clicked
 */
class TasksAdapter(
        private val context: Context,
        private val tasksList: List<Task>,
        private val checkFunc: (taskId: Int, isChecked: Boolean) -> Unit,
        private val deleteFunc: (taskId: Int) -> Unit
    ...
    override fun onBindViewHolder(holder: TaskViewHolder, position: Int) {
        ...
        itemView.cbTask.isEnabled = !task.isDone
        itemView.cbTask.setOnCheckedChangeListener { button, checked ->
        ...
        itemView.btnDeleteTask.setOnClickListener {
            val thisTask = tasksList[holder.adapterPosition]

            AlertDialog.Builder(context)
                    .setTitle(R.string.delete_task)
                    .setMessage("Are you sure that you want to delete \"${thisTask.description}\"?" +
                            " This action cannot be undone.")
                    .setPositiveButton(R.string.delete) { _, _ ->
                        deleteFunc(thisTask.id)
                    }
                    .setNeutralButton(R.string.cancel) { _, _ -> }
                    .show()
        }
